...
def CalculateMassMoRSE(lcoordinantes):
    ...
    R=_GetR(n=30)
    temp=[]
    mass=[get_atomicMass(coords[3]) for coords in lcoordinantes]
...
def CalculateAtomicNumberMoRSE(lcoordinates):
    ...
    R=_GetR(n=30)
    temp=[]
    mass = [get_atomicMass(coords[3]) for coords in lcoordinates]
    for i in lcoordinates:
        #if i[0]!='H':
        temp.append([float(i[0]),float(i[1]),float(i[2])])
    DM=_GetGementricalDistanceMatrix(temp)
...
def GetMorse3D(lcoordinates):

    dout = {}
    dout.update(CalculateUnweightMoRSE(lcoordinates))
    dout.update(CalculateChargeMoRSE(lcoordinates))
    dout.update(CalculateMassMoRSE(lcoordinates))
    dout.update(CalculateAtomicNumberMoRSE(lcoordinates))
    dout.update(CalculatePolarizabilityMoRSE(lcoordinates))
    dout.update(CalculateSandersonElectronegativityMoRSE(lcoordinates))
    dout.update(CalculateVDWVolumeMoRSE(lcoordinates))
    return dout
